...
const StoryUpload = () => {
  ...
  const uploadImage = async () => {
    ...
    setUploading(true);
    setTransferred(0);
    const task = storage().ref(`stories/${filename}`).putFile(uploadUri);
    // set progress state
  ...
  return (
    <View style={[tw('h-full bg-black'), {opacity: 0.89}]}>
      ...
      <TextInput
        placeholder={`${userData?.username} Please type your caption`}
        placeholderTextColor="gray"
        style={[tw('w-full px-2 h-12 bg-white text-black')]}
        type="text"
        value={caption}
